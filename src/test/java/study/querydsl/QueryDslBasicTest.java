...
public class QueryDslBasicTest {
    @PersistenceContext
    EntityManager em;
    // EntityManager 가 동시성 문제에 문제 없도록 설계되어 있다.
...
//                .select(member.count()) // = count(member.id)
    ...
    * */
    ...
    @Test
    void aggregation() {
        List<Tuple> result = queryFactory // Tuple -> 반환 타입이 여러개 일때 가끔 사용
                .select(
                        member.count(),
                        member.age.sum(),
                        member.age.avg(),
...
//        username_age
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(allEq(usernameCond, ageCond)) // null은 무시
                .fetch();
    }
    ...
    void bulkUpdate() {
        ...
        List<Member> result = queryFactory
                .selectFrom(member)
                .fetch();
        for (Member memberr : result) {
            System.out.println("memberr = " + memberr);
        }
    }
